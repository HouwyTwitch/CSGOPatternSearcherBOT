...
class LoginExecutor:
    ...
    def _prepare_login_request_data(self, encrypted_password: str, rsa_timestamp: str) -> dict:
        return {
            ...
            'captcha_text': self.captcha_code,
            'emailsteamid': '',
    ...
    def _check_for_captcha(login_response: requests.Response) -> None:
        if login_response.json().get('captcha_needed', False):
            self.captcha_gid = login_response.json()['captcha_gid']
            raise CaptchaRequired(f"Captcha required: https://steamcommunity.com/login/rendercaptcha/?gid={self.captcha_gid}")
